...
function createSpawnResult(overrides: Partial<RunCommandResult> = {}): RunCommandResult {
	return {
		stdout: "",
		stderr: "",
		status: 0,
...
): (command: string, args: string[]) => Promise<RunCommandResult> {
...
describe("pickDirectoryPathFromSystemDialog", () => {
	...
	// Regression: a picker with no display to show a window on (e.g. a headless
	// session) hangs forever instead of erroring. The child is killed after a
	// long safety-net timeout; that must surface as a clear error rather than
	// hanging the caller (previously spawnSync blocked the whole server for as
	// long as the hang lasted).
	it("throws a clear timeout error when the picker command is killed for taking too long", async () => {
		await expect(
			pickDirectoryPathFromSystemDialog({
				platform: "linux",
				runCommand: createRunCommand(
			...
			"Could not open directory picker via zenity: timed out waiting for a response (no display available?)",
		);
	});
