...
describe.sequential("runtime state stream integration", () => {
	...
	it("preserves completed review cards and their worktrees across shutdown", async () => {
		...
		try {
			...
			// A review task that already finished its turn is left untouched on shutdown: the card stays
			// in review (never trashed) and its session keeps awaiting_review so it can be resumed in place.
