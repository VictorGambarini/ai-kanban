...
- Editing per-task env applies at DIFFERENT moments depending on the card's state, and the UI surfaces it in two different places. A backlog card sets env from the inline editor (`TaskInlineCreateCard`, under Override Agent Settings) and it applies on the task's FIRST start. A running card (in_progress/review) sets env from the `TaskEnvButton` in the `CardDetailView` agent control bar — but env is baked into the PTY at spawn, so a live process can't pick it up; saving prompts a confirm dialog and, on confirm, RESTARTS the agent. Both surfaces hide the control when the effective agent is Cline (per-task env never reaches it). The restart is `restartTaskSessionEnv` (tRPC) → `TerminalSessionManager.restartTaskSessionWithEnv`: it re-resolves the effective env SERVER-SIDE from the hub config (the client only passes a taskId), updates the stored `restartRequest.env`, sets `forceRestartOnExit`, and stops the PTY so the existing exit→auto-restart path re-spawns with the new env (the agent resumes from its own on-disk session). `forceRestartOnExit` is consumed in `shouldAutoRestart` to bypass the crash-loop guard/listener checks and not consume the crash budget. Do NOT try to apply new env via `reloadTaskChatSession`/`reloadTaskSession` — that path is Cline-oriented and does NOT re-resolve env.
...
- Running the suites in a bare/sandboxed env (e.g. a fresh worktree over SSH with no `node_modules`) hits three non-obvious gates that all look like test failures but are environment setup, NOT your code. (1) **No deps**: `tsc`/`vitest` are missing because the env often has `NODE_ENV=production`, so `npm install` silently omits devDependencies — install with `npm install --include=dev` (and the same for `web-ui`). (2) **`localhost` doesn't resolve**: `/etc/hosts` may lack `localhost` and be read-only, so Vite/Vitest startup dies with `getaddrinfo ENOTFOUND localhost` (it's Vite resolving its dev-server host, not your test). Run web-ui tests with a throwaway config that overrides the host: `mergeConfig(base, { server: { host: "127.0.0.1" }, test: { api: { host: "127.0.0.1" }, environmentOptions: { jsdom: { url: "http://127.0.0.1/" } } } })` and pass it via `--config`. (3) **React in prod mode**: with `NODE_ENV=production`, React loads its production build and `act(...)` throws "act(...) is not supported in production builds of React" — prefix the run with `NODE_ENV=test`. So the working web-ui invocation is roughly `NODE_ENV=test npx vitest run --config <tmp>.config.ts <files>`. These are env-only workarounds — never commit the throwaway config or change the repo's real configs to "fix" them.
